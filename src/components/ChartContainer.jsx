...
export default function ChartContainer({
  ...
  useEffect(() => {
    ...
    const chartHeight = chartContainerRef.current.clientHeight || (isFullscreen ? window.innerHeight - 80 : 620);
  ...
  const chartEl = (
    ...
    >
      ...
      {chartSource === 'screenshot' ? (
        <div 
          className={`relative w-full flex items-center justify-center bg-slate-950 p-6 ${isFullscreen ? 'min-h-0 flex-1' : 'h-[620px]'}`}
        >
        ...
        <div
          ...
          className={`w-full bg-[#070b14] relative ${isFullscreen ? 'h-[calc(100vh-80px)]' : 'h-[620px]'}`}
        />
      )}
    </div>
  );
